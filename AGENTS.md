...
- **保存内容**: 1 リクエスト 1 エントリ。`timestamp` / `endpoint` / `provider` / `model` (`modelRequested`) / `stream` / `status` (`pending` → `ok` / `error`) / `inputTokens` / `outputTokens` / `durationMs` / `request` (プロンプト = system・messages・input・tools・tool_choice) / `response` (本文テキスト・ツール呼び出し) / `error`
...
- **閲覧ページ**: 一覧に Date / Model / Provider / Input / Output / Speed (= `outputTokens ÷ durationMs`、tok/s) を表示。行をクリックすると概要・プロンプト (ロール別)・レスポンス・生 JSON を右ペインに表示する。「更新」「自動更新 (3秒)」「クリア」操作あり
