...
<head>
  ...
  <title>ant2chat — Gemini API テスト</title>
...
<body>
  <main>
    <header>
      <h1><a href="/">ant2chat</a><span class="path"> /v1beta/models/{model}:generateContent</span></h1>
      <p>Google Gemini API (<code>generateContent</code> / <code>streamGenerateContent</code>) のテストコンソール。</p>
    </header>
    ...
    <section>
      ...
      <div class="form-group">
        <label for="f-model">model <span class="opt">（URL パスのモデル名。サーバーが --model 指定時は上書きされる）</span></label>
        <input type="text" id="f-model" value="gemini-2.5-flash" placeholder="例: gemini-2.5-flash" spellcheck="false" autocomplete="off">
      </div>

      <div class="form-group">
        <label for="f-system">systemInstruction <span class="opt">（任意）</span></label>
      ...
      <div class="form-row">
        <div class="form-group">
          <label for="f-max-tokens">maxOutputTokens</label>
      ...
      <p class="hint">Ctrl+Enter / ⌘+Enter でも送信できます。ストリーミングは <code>:streamGenerateContent?alt=sse</code> を使います。</p>
    </section>
